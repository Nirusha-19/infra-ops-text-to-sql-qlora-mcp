...
from common import SYSTEM_PROMPT, MODEL_NAME
from eval_utils import is_execution_match

from mlx_lm import load, generate

FEW_SHOT_EXAMPLES = [
    {"question": "How many services does the Compute Platform team own?",
     "sql": "SELECT COUNT(*) FROM services s JOIN teams t ON s.team_id = t.team_id WHERE t.name = 'Compute Platform';"},
    {"question": "List all team names.",
     "sql": "SELECT name FROM teams;"},
    {"question": "How many incidents are still unresolved?",
...
def main():
    ...
    print("First run downloads the model from Hugging Face (~4-5GB).")
    ...
    print(f"\n{'='*50}")
    print("Baseline (few-shot, no fine-tuning) execution accuracy")
    print(f"{'='*50}")
